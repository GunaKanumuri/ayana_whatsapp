import fs from 'fs';
const p = 'C:/Users/gunak/Downloads/ayana_whatsapp/frontend/src/pages/Dashboard.js';
let c = fs.readFileSync(p, 'utf8');

// 1. Update parent card to show schedule inline
const oldParentsList = `            {parents.length === 0 ? <EmptyState text="No parents added yet." /> : (
...
if (c.includes(oldParentsList)) {
  const newParentsList = `            {parents.length === 0 ? <EmptyState text="No parents added yet." /> : (
              <div className="grid sm:grid-cols-2 gap-4" data-testid="parents-list">
                {parents.map((p) => {
                  ...
                  <div key={p.id} className="bg-white rounded-xl border border-ayana-line p-5">
                    <div className="flex justify-between items-start">
                      ...
                      {p.nicknames && p.nicknames.length > 0 && (
                        <p className="text-xs text-ayana-muted mt-0.5">Nicknames: {p.nicknames.join(", ")}</p>
                      )}
                      {p.preferred_name && (
                        <p className="text-xs text-ayana-muted italic mt-0.5">Called &ldquo;{p.preferred_name}&rdquo; in messages</p>
                      )}
                    </div>
                    <div className="flex gap-1">
                        <SendTestDialog parent={p} categories={categories}
  ...
  c = c.replace(oldParentsList, newParentsList);
  console.log('Parents list updated with inline schedule info');
} else {
  console.error('oldParentsList not found');
  process.exit(1);
}
